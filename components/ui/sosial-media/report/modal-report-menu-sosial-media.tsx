...
const ModalReportMenuSosialMedia: React.FC<ModalReportMenuSosialMediaProps> = ({
  modalReportMenuSosialMedia,
  setModalReportMenuSosialMedia,
  setIndexMenuReportSosialMedia,
  setDataReportTypeContent,
    ...
    switch (id) {
      ...
      case 8:
        valueListReportTypeConten = {
          id: 8,
          textHeader:
            'What is the most appropriate description of the problem?',
          data: [
        ...
        >
          ...
          <View className="px-4" style={{ paddingTop: 20, paddingBottom: 10 }}>
            ...
            <Text style={{ color: '#8D8D8D' }}>
              All reports submitted are anonymous to protect your identity.
              However, in emergency situations that require immediate action, do
              not hesitate to contact emergency services.
            </Text>
          </View>
          <View className="px-4">
            {data.map((item, index) => {
              return (
                <View
                  key={index}
                  className="px-1"
                  style={{ borderBottomWidth: 1, borderBottomColor: '#E9E9E9' }}
                >
                  <ButtonReport
                    fontWeight={'500'}
                    title={item.text}
